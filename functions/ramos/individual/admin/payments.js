// Access-gated. Lists payments for one application.
export async function onRequestGet({ request, env }) {
  ...
  const { results } = await env.DB
    .prepare("SELECT id, amount_cents, payment_method, proof_original_filename, status, created_at FROM ramos_individual_payments WHERE application_id = ? ORDER BY created_at DESC")
    .bind(applicationId)
    .all();
